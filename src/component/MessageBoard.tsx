...
// 定義 MessageBoard 組件的 props 型別
interface MessageBoardProps {
  memberName: string;
}

function MessageBoard({ memberName }: MessageBoardProps): React.ReactElement {
  // 狀態管理
  const [messages, setMessages] = useState<Message[]>([]); // 儲存所有訊息，使用陣列結構存儲多筆留言數據
  const [content, setContent] = useState<string>(""); // 輸入框內容，用於控制表單輸入值
  const [user, setUser] = useState<User | null>(null); // 當前登入使用者，存儲 Firebase Auth 返回的用戶物件
  const [loading, setLoading] = useState<boolean>(true); // 是否正在載入，布林值控制載入狀態顯示
  const [isSubmitting, setIsSubmitting] = useState<boolean>(false);
  const messagesContainerRef = useRef<HTMLDivElement>(null); // 使用 useRef 建立對 DOM 元素的引用，用於控制訊息區塊自動滾動
  const MAX_MESSAGE_LENGTH: number = 100; // 最大字數限制
  ...
  */
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      toastStore.error("提交留言失敗：" + errorMessage);
    } finally {
          ...
          {loading ? (
            ...
            messages.map((message) => (
              <div
                ...
                className={`flex mb-4 ${
                  // 條件渲染：判斷訊息是自己的還是他人的
                  user && message.userId === user.uid ? "justify-end" : "justify-start"
                }`}
              >
                {/* 其他使用者的訊息 */}
